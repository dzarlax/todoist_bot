...
internal/
  ...
  todoist/
    ...
    server.go               # mcp-go MCPServer wiring 11 tools
...
Served at `POST /todoist` (and `/todoist/`) using `mcp-go`'s `StreamableHTTPServer`. Protected by `APIKeyAuth(cfg.APIKey)`. Eleven tools:
...
| `update_task` | Update a task (`task_id` required; optional `labels`) |
